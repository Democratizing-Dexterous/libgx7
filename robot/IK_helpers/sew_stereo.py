import numpy as np
from IK_helpers.subproblem import rot, vec_normalize
...
class sew_stereo:
    ...
    def fwd_kin(self, S, E, W):
        ...
        n_hat_ref = vec_normalize(np.cross(w_hat.flatten() - self.R.flatten(),
                                           self.V.flatten()).reshape(3,1))
    ...
    def jacobian(self, S, E, W):
        ...
        J_e = (np.cross(w_hat.flatten(), p_hat.flatten()).reshape(1,3) / np.linalg.norm(p)).T

        J_w_1 = ((w_hat.T @ self.V).item() / np.linalg.norm(x_c)) * y_hat_c
        J_w_2 = ((w_hat.T @ np.cross(self.R.flatten(), self.V.flatten()).reshape(3,1)).item() / np.linalg.norm(x_c)) * x_hat_c
        J_w_3 = ((w_hat.T @ E).item() / (np.linalg.norm(W) * np.linalg.norm(p))) * np.cross(w_hat.flatten(), p_hat.flatten()).reshape(3,1)

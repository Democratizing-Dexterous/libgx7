...
gx7_configs = load_config(os.path.join(os.path.dirname(abs_path), 'configs/gx7_test.yaml'))
...
class RobotMotors: # https://gl1po2nscb.feishu.cn/wiki/VYrlwHI7liHzXIkx0s0cUOVdnzb
    ...
    def set_zero(self, id):
        ...
        return extract_feedback_frame(MOTOR_TYPES, MOTOR_CONFIGS[id-1], feedback_frame) # id start from 1
    ...
    def enable_motor(self, id):
        ...
        return extract_feedback_frame(MOTOR_TYPES, MOTOR_CONFIGS[id-1], feedback_frame)
    ...
    def set_motor_mit(self, id, pos, vel, kp, kd, torque):
        torq_control_frame = make_mit_frame(MOTOR_TYPES, MOTOR_CONFIGS[id-1], pos, vel, kp, kd, torque) 

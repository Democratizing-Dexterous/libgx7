...
class VCI_CAN_OBJ_ARRAY(Structure):
    ...
    def __init__(self, num_of_structs):
        self.STRUCT_ARRAY = cast((VCI_CAN_OBJ * num_of_structs)(),
                                 POINTER(VCI_CAN_OBJ))
...
class CAN:
    def __init__(self):
        self.lib_can_path = os.path.join(os.path.dirname(abs_path), 'libs/ControlCAN.dll')
        self.canDLL = windll.LoadLibrary(self.lib_can_path)

    def initCan1(self):
        vci_initconfig = VCI_INIT_CONFIG(0x80000008, 0xFFFFFFFF, 0, 0, 0x00,
                                         0x14, 0)  #1M bps
    ...
    def send_frame(self, id, hex_list, timeout=0):
        ...
        if ret:
            while True:
                ret = self.canDLL.VCI_Receive(VCI_USBCAN2, 0, 0,
                                                byref(rx_vci_can_obj.ADDR), 2500,
                                                0)
                if ret:
...
def make_mit_frame(motor_types, motor_name, pos, vel, kp, kd, tor):
    ...
    kp_tmp = float_to_uint(kp, KP_MIN, KP_MAX, 12)  # 假设 KP_MIN 和 KP_MAX 是预定义的常量
    kd_tmp = float_to_uint(kd, KD_MIN, KD_MAX, 12)  # 假设 KD_MIN 和 KD_MAX 是预定义的常量

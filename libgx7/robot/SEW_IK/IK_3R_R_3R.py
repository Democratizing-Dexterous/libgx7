...
def IK_3R_R_3R(R_07, p_0T, SEW_class, psi, kin):
    ...
    t4, t4_is_LS = subproblem.sp_3(
        kin["P"][:, 4], -kin["P"][:, 3], kin["H"][:, 3], np.linalg.norm(p_SW)
    )
    ...
    for q4 in t4.flatten():
        ...
        t_a, t_a_is_LS = subproblem.sp_4(
            n_SEW, rot(n_SEW, theta_b) @ rot(e_SW, theta_c) @ kin["P"][:, 3], e_SW, 0
        )
        ...
        for theta_a in t_a.flatten():
            ...
            t2, t1, t12_is_LS = subproblem.sp_2(
                kin["H"][:, 2], R_03 @ kin["H"][:, 2], kin["H"][:, 1], -kin["H"][:, 0]
            )

            for q1, q2 in zip(t1.flatten(), t2.flatten()):
                q3, q3_is_LS = subproblem.sp_1(
                    p_spherical_1,
                    (rot(kin["H"][:, 0], q1) @ rot(kin["H"][:, 1], q2)).T
                    @ R_03
                    @ p_spherical_1,
                    kin["H"][:, 2].reshape(3, 1),
                )
                ...
                for q5, q6 in zip(t5.flatten(), t6.flatten()):
                    q7, q7_is_LS = subproblem.sp_1(
                        p_spherical_2,
                        (rot(kin["H"][:, 4], q5) @ rot(kin["H"][:, 5], q6)).T
                        @ R_47
                        @ p_spherical_2,
                        kin["H"][:, 6],
                    )
